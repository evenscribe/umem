...
const TUNNEL_ADDRESS: &str = "https://mcp.evenscribe.com";
...
impl McpOAuthStore {
    ...
    async fn validate_client(
        ...
        debug!("Available clients: {:?}", clients.keys().collect::<Vec<_>>());
        debug!("Looking for client_id: {}, redirect_uri: {}", client_id, redirect_uri);
        
        if let Some(client) = clients.get(client_id) {
            debug!("Found client config: {:?}", client);
            // Allow any localhost redirect URI for mcp-client
            if client_id == "mcp-client" && (redirect_uri.starts_with("http://localhost") || redirect_uri.starts_with("https://localhost")) {
        ...
        // Allow any client_id that starts with "client-" and uses claude.ai redirect
        if client_id.starts_with("client-") && redirect_uri == "https://claude.ai/api/mcp/auth_callback" {
            warn!("Allowing Claude Desktop client {} with redirect {}", client_id, redirect_uri);
            let client_config = OAuthClientConfig {
                client_id: client_id.to_string(),
                client_secret: Some("claude-desktop-secret".to_string()),
                scopes: vec!["profile".to_string(), "email".to_string()],
                redirect_uri: redirect_uri.to_string(),
            };
            
            // Register this client for future use
            drop(clients); // Release the read lock
            self.clients.write().await.insert(client_id.to_string(), client_config.clone());
            
            return Some(client_config);
        }
        
        // For debugging: allow any client_id that starts with "client-" and any localhost redirect
        if client_id.starts_with("client-") && (redirect_uri.starts_with("http://localhost") || redirect_uri.starts_with("https://localhost")) {
            warn!("DEBUG: Allowing unregistered client {} with redirect {}", client_id, redirect_uri);
            return Some(OAuthClientConfig {
                client_id: client_id.to_string(),
                client_secret: Some("debug-secret".to_string()),
                scopes: vec!["profile".to_string(), "email".to_string()],
                redirect_uri: redirect_uri.to_string(),
            });
        }
...
async fn oauth_authorize(
    ...
    info!("OAuth authorize request - client_id: {}, redirect_uri: {}, scope: {:?}, state: {:?}", 
          params.client_id, params.redirect_uri, params.scope, params.state);
        ...
        .await
    {
        ...
        error!("Client validation failed - client_id: {}, redirect_uri: {}", params.client_id, params.redirect_uri);
        (
...
async fn oauth_token(
    ...
    // validate client
    info!("Validating client: {} with redirect_uri: {}", client_id, token_req.redirect_uri);
    match state
        ...
        .await
    {
        Some(_) => {
            ...
            info!("Client validated successfully. Got session id: {}", session_id);
...
async fn validate_token_middleware(
    ...
    info!("SSE endpoint accessed: {} {}", request.method(), request.uri());
    ...
    // Validate the token
    debug!("Validating token: {}", &token[..std::cmp::min(token.len(), 20)]);
    match token_store.validate_token(&token).await {
        Some(valid_token) => {
            debug!("Token validated successfully for client: {}", valid_token.client_id);
            next.run(request).await
        },
        None => {
            warn!("Token validation failed for token: {}", &token[..std::cmp::min(token.len(), 20)]);
...
async fn oauth_authorization_server() -> impl IntoResponse {
    ...
    additional_fields.insert(
        ...
        Value::Array(vec![Value::String("S256".into()), Value::String("plain".into())]),
    );
    additional_fields.insert(
        "grant_types_supported".into(),
        Value::Array(vec![Value::String("authorization_code".into())]),
    );
    additional_fields.insert(
    ...
    let metadata = AuthorizationMetadata {
        authorization_endpoint: format!("{}/oauth/authorize", TUNNEL_ADDRESS),
        token_endpoint: format!("{}/oauth/token", TUNNEL_ADDRESS),
        scopes_supported: Some(vec!["profile".to_string(), "email".to_string()]),
        registration_endpoint: format!("{}/oauth/register", TUNNEL_ADDRESS),
        issuer: Some(TUNNEL_ADDRESS.to_string()),
        jwks_uri: Some(format!("{}/oauth/jwks", TUNNEL_ADDRESS)),
        additional_fields,
    };
...
pub async fn run_server() -> Result<()> {
    ...
    let oauth_server_router = Router::new()
        .route(
            ...
            get(oauth_authorization_server)
                .post(oauth_authorization_server)
    ...
    let app = Router::new()
        ...
        .route("/mcp", get(index).post(index))
    ...
    info!("Registering MCP service with SSE server");
